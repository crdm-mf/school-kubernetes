...
Alle Events verwenden `event_id`, `event_version`, Zeitstempel, `correlation_id`, `causation_id`, `source` und einen
typisierten Payload. RabbitMQ liefert at-least-once; PostgreSQL verhindert doppelte Verarbeitung über
`processed_events`.

Die Courier-Position liegt in jeder Phase auf dem gemeinsamen Strassengraphen. `progress` verwendet `0.0-0.45` fuer die
Anfahrt zum Restaurant, `0.5` fuer die Abholung und `0.5-1.0` fuer die Fahrt zum Kunden.

## Optionale technische Korrelation

Die optionale Distributed-Tracing-Erweiterung ergänzt diese fachlichen Events nicht um neue Payload-Felder.
`correlation_id` bleibt die Order-ID und wird als
`food_delivery.correlation_id` an den Trace-Spans sichtbar gemacht. Der technische W3C-Kontext wird ausschließlich über
die AMQP-Header `traceparent`
und `tracestate` propagiert. Dadurch können fachliche Event-Suche und technische Trace-Suche getrennt erklärt und
trotzdem für dieselbe Bestellung verbunden werden. Beispiele stehen in [docs/tracing.md](tracing.md).
