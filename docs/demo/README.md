...
Die Aufnahmen dokumentieren den Pflichtstand bis Block 7. Distributed Tracing ist eine separat aktivierbare optionale
Erweiterung und wird deshalb nicht als eigener Kursblock vorausgesetzt.
...
| [09-tracing.png](09-tracing.png)                                             | Dashboard mit ausgewählter abgeschlossener Bestellung und Grafana Explore: TraceQL-Suche über die vollständige `correlation_id`, durchgehender Trace über fünf Services sowie RabbitMQ- und PostgreSQL-Spans.                                              |
...
## Optionale Tracing-Demo

Der folgende Ablauf ergänzt den Pflichtstand um einen nachvollziehbaren End-to-End-Trace. Er ist kein eigener Kursblock.
...
1. Erzeuge eine Bestellung im Dashboard unter <http://localhost:8080>.
2. Warte, bis die Bestellung `Geliefert` oder `Fehlgeschlagen` ist. Das External-Link-Symbol erscheint dann an der
   Bestellung und führt zu Grafana Explore mit der vollständigen Order-ID:

   ```traceql
   { span.food_delivery.correlation_id = "ORDER_ID" }
   ```

3. Halte den geöffneten Trace fest. Sichtbar sein sollen mindestens der
   `POST /api/v1/orders`-Span, RabbitMQ-Producer- und Consumer-Spans sowie die PostgreSQL-Spans des Order Workers. Bei
   einer fehlgeschlagenen Bestellung endet der Ablauf mit `order.rejected` statt `order.delivered`; die gleiche
   Order-ID-Suche funktioniert unverändert.

Damit zeigt die Demo sowohl die fachliche Korrelation über die Order-ID als auch den technischen Ablauf über Control
API, RabbitMQ, Worker und PostgreSQL. Die Aufnahme [09-tracing.png](09-tracing.png) zeigt diesen Ablauf vom
Dashboard-Link bis zum geöffneten Trace in Grafana Explore. Details zu Setup, weiteren TraceQL-Abfragen und Grenzen
stehen in
